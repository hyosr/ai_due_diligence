# # app/services/runner.py (version avec ML)
# from sqlalchemy.orm import Session
# from app.models.models import Service, Assessment
# from app.services.collector import collect_runtime_data
# from app.services.features import extract_features
# from app.services.scorer import score_risk          # on garde pour la compatibilité, mais on va utiliser ML
...
#         # 1) Collecte runtime (headers, etc.)
...
#         # 6) Pass‑through pour l’extracteur de features (optionnel)
...
#         # Construction du dictionnaire de features attendu par le modèle ML
#         ml_features = {
#             "has_https": 1 if s.url.startswith("https") else 0,
#             "ssl_valid": 1 if ssl_info.get("ssl_valid") else 0,
#             "auth_method": s.auth_method or "none",
#             "num_vulns": payload.get("num_known_vulnerabilities", 0),
#             "reputation": payload.get("reputation_score_external", 0.5),
#             "compliance_score": 0.9 if payload.get("gdpr_compliant", False) else 0.3
#         }

#         # Appel au modèle ML
#         ml_result = predict_risk(ml_features)
#         ml_reasons = explain_prediction(ml_features)

#         # ========== Récupération du scoring linéaire (optionnel, pour compatibilité) ==========
#         features = extract_features(payload, collected)   # features originales (pour explicabilité)
#         scored = score_risk(features)                     # scoring linéaire (gardé pour référence)

#         # On remplace le risque par celui du ML
#         scored["risk_score"] = ml_result["risk_score"]
#         scored["risk_level"] = ml_result["risk_level"]

#         # On enrichit les raisons avec celles du ML
#         reasons = ml_reasons   # on utilise directement les raisons du ML

#         # ========== Application du Policy Engine (règles dures) ==========
...
#         # ========== Construction de l’explicabilité ==========
...
#         explain["ml_features"] = ml_features          # ajout des features utilisées par le ML
#         explain["ml_probability"] = ml_result["risk_score"]

#         # ========== Mise à jour de la base ==========
#         a.risk_score = scored["risk_score"]
#         a.risk_level = scored["risk_level"]
#         a.decision = final_decision
#         a.confidence = scored.get("confidence", 0.85)
...
async def run_assessment_job(db: Session, assessment_id: int):
    ...
    try:
        ...
        # 3) Reputation connector (VirusTotal/fallback)
        rep_info = await reputation_lookup(s.url)
        collected["reputation_external"] = rep_info
        if payload.get("reputation_score_external") is None:
            payload["reputation_score_external"] = rep_info.get("reputation_score_external", 0.5)
        ...
        # 4) Security headers connector  <-- put here
        headers_info = await scan_security_headers(s.url)
        collected["security_headers"] = headers_info

        # 5) Domain age connector        <-- put here
        domain_info = get_domain_age_signal(s.url)
        collected["domain_age"] = domain_info

        # 6) Pass-through values for feature extractor
        payload["security_headers_score"] = headers_info.get("security_headers_score")
        payload["domain_age_risk"] = domain_info.get("domain_age_risk")
